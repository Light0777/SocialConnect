...
import { ProfileContext, UserProfile } from "@/contexts/ProfileContext";
...
export default function CreatePostScreen() {
  ...
  // Track dimensions for the slider
  const sliderTrackRef = useRef(null);
  const [maxSlideValue, setMaxSlideValue] = useState(190); // Default fallback value

  // Measure the slider track to get accurate max slide value
  const handleTrackLayout = (event: { nativeEvent: { layout: { width: any; }; }; }) => {
    const { width: trackWidth } = event.nativeEvent.layout;
    const thumbWidth = 60; // Your thumb width
    setMaxSlideValue(trackWidth - thumbWidth);
  };
  ...
  const handlePublish = async () => {
    ...
    if (currentTitle.length === 0 || currentDescription.length === 0) {
      ...
      Animated.spring(slideAnim, { toValue: 0, useNativeDriver: false, friction: 8 }).start();
      return;
    }
    ...
    try {
      ...
      const postContent = {
        title: currentTitle,                     // ✅ Added
        description: currentDescription,         // ✅ Added
        userId: profile.id,                      // ✅ Added
        userName: profile.name || "Anonymous",
        bio: profile.bio || "",
        profileImg: profile.profileImg || null,  // ✅ key matches backend
      ...
      <View style={{
        ...
        {profile.bgImage ? (
          ...
          <View style={{
            ...
            left: 0,
            right: 0,
            backgroundColor: profile.bgColor || "#141414"
          }} />
        )}
        <View
          style={{
        ...
        />
        <Image
          source={{ uri: profile.profileImg || "https://placehold.co/200x200" }}
          style={{
            width: 110,
            height: 110,
            borderRadius: 65,
        ...
        <Text style={styles.profileBio}>{profile.bio || "No bio added yet"}</Text>
